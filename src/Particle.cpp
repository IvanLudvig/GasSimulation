...
Particle::Particle(const double mass, const double radius, const vector3D &pos,
    ...
    : mass{mass}, radius{radius}, pos{pos}, speed{speed}, acceleration{
                                                              acceleration} {}
...
void Particle::collideWithParticle(Particle &p) {
  ...
  vector3D temp = (e * (p.getSpeed() - speed) * e /
                   ((p.getMass() + mass) * e.length() * e.length())) *
                  2;
  speed += p.getMass() * temp;
  p.setSpeed(p.getSpeed() - mass * temp);
}

void Particle::setMass(const double mass) { this->mass = mass; }

void Particle::setU(const double U) { this->U = U; }

void Particle::setPos(const vector3D &pos) { this->pos = pos; }

void Particle::setSpeed(const vector3D &speed) { this->speed = speed; }

void Particle::setAcceleration(const vector3D &acceleration) {
  this->acceleration = acceleration;
}

void Particle::setForce(vector3D force) { this->acceleration = force / mass; }

void Particle::addForce(vector3D force) { this->acceleration += force / mass; }

double Particle::getU() const { return U; }

void Particle::addU(double U) { this->U += U; }

double Particle::getE() const { return E; }

double Particle::getMass() { return mass; }

vector3D Particle::getPos() const { return pos; }

vector3D Particle::getSpeed() const { return speed; }

vector3D Particle::getAcceleration() const { return acceleration; }

vector3D Particle::getForce() const { return mass * acceleration; }
